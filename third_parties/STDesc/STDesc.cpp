...
{
  // pre-preocess
  nh.param<int>("/multiProxy/STDParam/maximum_corner_num", config_setting.maximum_corner_num_, 100);

  // key points
  nh.param<double>("/multiProxy/STDParam/plane_merge_normal_thre", config_setting.plane_merge_normal_thre_, 0.1);
  nh.param<double>("/multiProxy/STDParam/plane_detection_thre", config_setting.plane_detection_thre_, 0.01);
  nh.param<double>("/multiProxy/STDParam/voxel_size", config_setting.voxel_size_, 2.0);
  nh.param<int>("/multiProxy/STDParam/voxel_init_num", config_setting.voxel_init_num_, 10);
  nh.param<double>("/multiProxy/STDParam/proj_image_resolution", config_setting.proj_image_resolution_, 0.5);
  nh.param<double>("/multiProxy/STDParam/proj_dis_min", config_setting.proj_dis_min_, 0);
  nh.param<double>("/multiProxy/STDParam/proj_dis_max", config_setting.proj_dis_max_, 2);
  nh.param<double>("/multiProxy/STDParam/corner_thre", config_setting.corner_thre_, 10);

  // std descriptor
  nh.param<int>("/multiProxy/STDParam/descriptor_near_num", config_setting.descriptor_near_num_, 10);
  nh.param<double>("/multiProxy/STDParam/descriptor_min_len", config_setting.descriptor_min_len_, 2);
  nh.param<double>("/multiProxy/STDParam/descriptor_max_len", config_setting.descriptor_max_len_, 50);
  nh.param<double>("/multiProxy/STDParam/non_max_suppression_radius", config_setting.non_max_suppression_radius_, 2.0);
  nh.param<double>("/multiProxy/STDParam/std_side_resolution", config_setting.std_side_resolution_, 0.2);

  // candidate search
  nh.param<int>("/multiProxy/STDParam/skip_near_num", config_setting.skip_near_num_, 50);
  nh.param<int>("/multiProxy/STDParam/candidate_num", config_setting.candidate_num_, 50);
  nh.param<int>("/multiProxy/STDParam/sub_frame_num", config_setting.sub_frame_num_, 10);
  nh.param<double>("/multiProxy/STDParam/vertex_diff_threshold", config_setting.vertex_diff_threshold_, 0.5);
  nh.param<double>("/multiProxy/STDParam/rough_dis_threshold", config_setting.rough_dis_threshold_, 0.01);

  // RTverify
  nh.param<double>("/multiProxy/STDParam/RTthreshold", config_setting.RTthreshold, 3.0);
  nh.param<int>("/multiProxy/STDParam/maxvotethre", config_setting.maxvotethre, 10);

  std::cout << "Sucessfully load parameters:" << std::endl;
  std::cout << "----------------Main Parameters-------------------"
            << std::endl;
  std::cout << "maxvotethre:" << config_setting.maxvotethre << std::endl;
...
void STDescManager::GenerateSTDescs(
  ...
  corner_points.reset(new pcl::PointCloud<pcl::PointXYZINormal>);
  corner_extractor(voxel_map, input_cloud, corner_points);

//  std::cout << "Corner count: " << corner_points->size() << std::endl;
...
void STDescManager::SearchLoop(
    const std::vector<STDesc> &stds_vec, int frame_id, std::pair<int, double> &loop_result,
    ...
    std::vector<std::pair<STDesc, STDesc>> &loop_std_pair)
{
  ...
  candidate_selector(stds_vec, candidate_matcher_vec, frame_id);
  ...
  for (auto & i : candidate_matcher_vec)
  {
    double verify_score = -1;
    std::pair<Eigen::Vector3d, Eigen::Matrix3d> relative_pose;
    std::vector<std::pair<STDesc, STDesc>> sucess_match_vec;
    candidate_verify(i, verify_score, relative_pose,
                     sucess_match_vec);
    if (verify_score > best_score)
    {
      best_score = verify_score;
      best_candidate_id = i.match_id_.second;
      loop_result = std::pair<int, double>(best_candidate_id, verify_score);
      loop_transform = relative_pose;
      loop_std_pair = sucess_match_vec;
    }
  }
...
{
  // update frame id
  std::lock_guard<std::mutex> lock(mtx_database);
    ...
    std::unordered_map<STD_VOXEL_LOC, STDOctoTree *> &voxel_map)
{
  ...
  for (uint i = 0; i < plsize; i++) {
    ...
    STD_VOXEL_LOC position((int64_t)loc_xyz[0], (int64_t)loc_xyz[1],
                           (int64_t)loc_xyz[2]);
    ...
    {
      STDOctoTree *octo_tree = new STDOctoTree(config_setting_);
      voxel_map[position] = octo_tree;
      voxel_map[position]->voxel_points_.push_back(p_c);
    }
  }
  std::vector<std::unordered_map<STD_VOXEL_LOC, STDOctoTree *>::iterator> iter_list;
    ...
    std::unordered_map<STD_VOXEL_LOC, STDOctoTree *> &voxel_map)
{
  for (auto iter = voxel_map.begin(); iter != voxel_map.end(); iter++) {
    if (iter->second->plane_ptr_->is_plane_) {
      STDOctoTree *current_octo = iter->second;
      for (int i = 0; i < 6; i++) {
        STD_VOXEL_LOC neighbor = iter->first;
        if (i == 0) {
        ...
        if (near == voxel_map.end()) {
          ...
          if (!current_octo->is_check_connect_[i]) {
            STDOctoTree *near_octo = near->second;
            current_octo->is_check_connect_[i] = true;
            int j;
            if (i >= 3) {
              j = i - 3;
...
void STDescManager::getPlane(
    const std::unordered_map<STD_VOXEL_LOC, STDOctoTree *> &voxel_map,
    ...
    pcl::PointCloud<pcl::PointXYZINormal>::Ptr &corner_points)
{
  ...
  for (auto iter = voxel_map.begin(); iter != voxel_map.end(); iter++) {
    if (!iter->second->plane_ptr_->is_plane_) {
      STD_VOXEL_LOC current_position = iter->first;
      STDOctoTree *current_octo = iter->second;
      int connect_index = -1;
      for (int i = 0; i < 6; i++) {
        if (current_octo->connect_[i]) {
          connect_index = i;
          STDOctoTree *connect_octo = current_octo->connect_tree_[connect_index];
          bool use = false;
          for (int j = 0; j < 6; j++) {
          ...
          if (current_octo->voxel_points_.size() > 10) {
            ...
            for (auto voxel_inc : voxel_round) {
              STD_VOXEL_LOC connect_project_position = current_position;
...
void STDescManager::build_stdesc(
  ...
  std::unordered_map<STD_VOXEL_LOC, bool> feat_map;
  ...
  for (size_t i = 0; i < corner_points->size(); i++) {
    ...
    for (int m = 1; m < near_num - 1; m++) {
      for (int n = m + 1; n < near_num; n++) {
        ...
        double a = sqrt(pointNKNSquaredDistance[m]);
        double b = sqrt(pointNKNSquaredDistance[n]);
        if (a > max_dis_threshold || b > max_dis_threshold ||
            a < min_dis_threshold || b < min_dis_threshold) {
          continue;
        }
        ...
        double c = sqrt(pow(p3.x - p2.x, 2) + pow(p3.y - p2.y, 2) +
                        pow(p3.z - p2.z, 2));
        if (c > max_dis_threshold || c < min_dis_threshold) {
          continue;
        }
        ...
        d_p.y = b * 1000;
        d_p.z = c * 1000;
        STD_VOXEL_LOC position((int64_t)d_p.x, (int64_t)d_p.y, (int64_t)d_p.z);
        ...
        if (iter == feat_map.end()) {
          ...
          single_descriptor.frame_id_ = frame_id;
...
void STDescManager::candidate_selector(
  ...
  std::unordered_map<int, double> match_array;
      ...
      if ((src_std.side_length_ - voxel_center).norm() < 1.5) {
        ...
        if (iter != data_base_.end()) {
          for (size_t j = 0; j < data_base_[position].size(); j++) {
            if (abs((int)(src_std.frame_id_ - data_base_[position][j].frame_id_)) < config_setting_.skip_near_num_)
              continue;
            ...
            if (dis < dis_threshold) {
              ...
              double vertex_attach_diff = 2.0 * (src_std.vertex_attached_ - data_base_[position][j].vertex_attached_).norm() /
...
//   std::cout << "dis match num:" << dis_match_cnt << ", final match num:" << final_match_cnt << std::endl;
  ...
  for (int cnt = 0; cnt < config_setting_.candidate_num_; cnt++) {
    ...
    if (max_vote_index >= 0 && max_vote >= std::min(5, config_setting_.maxvotethre)) {
      match_array[max_vote_index] = 0;
      match_triangle_list.match_id_.first = frame_id;
...
void STDescManager::candidate_verify(
  ...
  double dis_threshold = config_setting_.RTthreshold;
...
#pragma omp parallel for
  ...
  if (max_vote >= config_setting_.maxvotethre) {
...
void STDOctoTree::init_octo_tree()
{
